...
class ScratchEditorState extends FlxState {
    ...
    var nombreScript:String = "MiScript";
    ...
    function getCategoriaColor(cat:String, seleccionado:Bool):Int {
        return switch (cat) {
            case "event": seleccionado ? 0xFFFFD600 : 0xFFBCA800; // Amarillo (seleccionado) y amarillo oscuro
            case "sensores": seleccionado ? 0xFF80D8FF : 0xFF0091EA; // Azul claro (seleccionado) y azul oscuro
            default: seleccionado ? 0xFF8000FF : 0xFF333333;
        }
    }

    // Devuelve el color principal de la categoría (para el área de código)
    function getCategoriaMainColor(cat:String):Int {
        return switch (cat) {
            case "event": 0xFFFFD600; // Solo amarillo para eventos
            case "sensores": 0xFF80D8FF; // Solo azul claro para sensores
            default: 0xFFFFFFFF; // Blanco por defecto para otras categorías
        }
    }
    ...
    override public function create():Void {
        ...
        var titulo = new FlxText(460, 30, 0, "📜 Área de Scripts");
        ...
        inputNombre = new FlxText(520, 60, 200, nombreScript);
        inputNombre.setFormat(null, 14, FlxColor.YELLOW);
        add(inputNombre);

        // --- Categorías ---
        categorias = Blocks.getCategorias();
        ...
        // --- Botón para volver ---
        var btnGuardar = new FlxButton(FlxG.width - 240, 10, "💾 Guardar", guardarScript);
        add(btnGuardar);
    ...
    override public function update(elapsed:Float):Void {
        ...
        if (FlxG.mouse.justPressed) {
            ...
            for (i in 0...bloquesDisponibles.length) {
                ...
                if (bloque.overlapsPoint(FlxG.mouse.getPosition())) {
                    ...
                    var bloqueInfo = Blocks.getBloquesPorCategoria(categoriaSeleccionada)[i];
                    var clon = crearBloqueVisual(bloqueInfo, bloque.x, bloque.y);
                    add(clon); // Agregar el clon
        ...
        if (bloqueArrastrado != null) {
            if (FlxG.mouse.pressed) {
                ...
                if (indice == -1) {
                    ...
                    if (ultimoTexto != null) {
                        ultimoTexto.x = nuevaX + 10;
                        ultimoTexto.y = nuevaY + 12;
                    }
                } else {
                    ...
                    for (i in 0...textosBloque.length) {
                        ...
                        if (texto != null) {
                            // Verificar si este texto pertenece al bloque arrastrado
                ...
                if (scriptArea.overlapsPoint(FlxG.mouse.getPosition())) {
                    // Mantener el clon en el área de scripts
                    var indiceExistente = bloquesEnScript.indexOf(bloqueArrastrado);
                    if (indiceExistente == -1) {
                        bloquesEnScript.push(bloqueArrastrado);
                    }
                    ordenarBloques(); // Ordenar bloques por posición
                    dibujarConectores(); // Dibujar conectores visuales
                } else {
                    // Eliminar el clon y su texto si no se suelta en el área de scripts
        ...
        // --- Ejecutar bloques con doble clic ---
        if (FlxG.mouse.justPressed) {
            for (i in 0...bloquesEnScript.length) {
                var bloque = bloquesEnScript[i];
                if (bloque.overlapsPoint(FlxG.mouse.getPosition())) {
    ...
    function crearBloqueVisual(bloque:BloqueInfo, x:Float, y:Float):FlxSprite {
        ...
        for (py in 0...alto) {
            for (px in 0...ancho) {
                ...
                // Conector superior (muesca)
                else if (py >= 0 && py < 8 && px >= 20 && px < 60) {
                    dibujar = true;
                }
                // Conector inferior (saliente)
    ...
    function ordenarBloques():Void {
        // Buscar bloques "al empezar la canción" para conectar con otros
        for (i in 0...bloquesEnScript.length) {
    ...
    // --- Actualizar posición del texto de un bloque ---
    ...
    // --- Refresca la lista de bloques según la categoría seleccionada ---
    function refrescarListaBloques():Void {
        if (listaBloques != null) remove(listaBloques, true);
        
        // Crear área de bloques
        listaBloques = new FlxSprite(130, 20);
        listaBloques.makeGraphic(300, FlxG.height - 40, 0xFF404040);
        add(listaBloques);
        
        var tituloBloques = new FlxText(140, 30, 0, "🧩 Bloques: " + categoriaSeleccionada);
        tituloBloques.setFormat(null, 14, FlxColor.WHITE);
        add(tituloBloques);
        
        // Obtener bloques de la categoría
        var bloques = Blocks.getBloquesPorCategoria(categoriaSeleccionada);
        var y = 60;
        ...
        for (bloque in bloques) {
            ...
            // Crear texto para bloque de paleta
    ...
    function guardarScript():Void {
        // Actualizar nombre desde el input
        nombreScript = inputNombre.text;
        ...
        try {
            ...
            var scriptContent = "// Script generado desde ScratchEditorState\n";
            scriptContent += "function customScript() {\n";
            ...
            for (i in 0...bloquesEnScript.length) {
                ...
                if (texto != null) {
                    ...
                    scriptContent += "    trace(\"Ejecutando: " + texto.text + "\");\n";
                    eventCount++;
                }
            }
            ...
            var fileName = nombreScript + ".hx";
            
            if (FileSystem.exists(scriptsPath + "/" + fileName)) {
                mostrarDialogoReemplazo(scriptsPath + "/" + fileName, scriptContent);
                return;
            }
    ...
    function mostrarDialogoReemplazo(fileName:String, data:String):Void {
        ...
        var dialogText = new FlxText(confirmDialog.x + 20, confirmDialog.y + 20, 360, 
            "El script '" + nombreScript + ".hx' ya existe.\n\n¿Quieres reemplazarlo?");
